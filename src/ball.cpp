...
Ball::Ball(float x, float y, color_t color) {
    ...
    for (i = 0; i < n; i++)
    {
        ...
        vertex_buffer_data[9 * i + 3] = (double)cos((2 * M_PI * i)/n);
        vertex_buffer_data[9 * i + 4] = (double)sin((2 * M_PI * i)/n);
        vertex_buffer_data[9 * i + 5] = 2.0f;
        
        vertex_buffer_data[9 * i + 6] = (double)cos((2 * M_PI * (i + 1))/n);
        vertex_buffer_data[9 * i + 7] = (double)sin((2 * M_PI * (i + 1))/n);
